...
class Loss(nn.Module):
    def __init__(
        self,
        feature_size=7,  # 피처맵 사이즈
        num_bboxes=2,
        num_classes=20,
        lambda_coord=5.0,  # bbox의 location과 size에 관한 weight
        lambda_noobj=0.5,  # no object loss의 weight
    ):
    ...
    def compute_iou(self, bbox1: torch.Tensor, bbox2: torch.Tensor):
        N = bbox1.size(0)
        M = bbox2.size(0)

        # compute left-top coordinate of intersections
        lt = torch.max(
            bbox1[:, :2]
            .unsqueeze(1)
        ...
        # compute area of the intersections form the coordinates
        wh = rb - lt  # width and height of intersections [N,M,2]
        wh[wh < 0] = 0  # clip at 0
        inter = wh[:, :, 0] * wh[:, :, 1]  # [N, M]

        area1 = (bbox1[:, 2] - bbox1[:, 0]) * (bbox1[:, 3] - bbox1[:, 1])  # [N,]
        area2 = (bbox2[:, 2] - bbox2[:, 0]) * (bbox2[:, 3] - bbox2[:, 1])  # [M,]
        area1 = area1.unsqueeze(1).expand_as(inter)  # [N,1] -> [N, M]
        area2 = area2.unsqueeze(0).expand_as(inter)  # [1,M] -> [N, M]
    ...
    def forward(self, pred_tensor: torch.Tensor, target_tensor: torch.Tensor):
        ...
        N = 5 * B + C  # 5 is [x, y, w, h, conf]

        batch_size = pred_tensor.size(0)
        coord_mask = (
            target_tensor[:, :, :, 4] > 0
        )  # mask for the cells which contain objs [n_batch,S,S]
        noobj_mask = (
            target_tensor[:, :, :, 4] == 0
        )  # mask for the cells which do not contain objs [n_batch,S,S]
        ...
        bbox_pred = (
            coord_pred[:, : 5 * B].contiguous().view(-1, 5)
        )  # [n_coord X B, 5=len([x,y,w,h,conf])]
        class_pred = coord_pred[:, 5 * B :]  # [n_coord, C]

        coord_target = target_tensor[coord_mask].view(-1, N)  # [ncoord, N]

        bbox_target = (
            coord_target[:, : 5 * B].contiguous().view(-1, 5)
        )  # [n_coord X B, 5=len([x,y,w,h,conf])]
        ...
        # compute loss for the cells with objs
        coord_response_mask = torch.cuda.ByteTensor(bbox_target.size()).fill_(
            0
        )  # [n_coord x B, 5]
        coord_not_response_mask = torch.cuda.ByteTensor(bbox_target.size()).fill_(
            1
        )  # [n_coord x B, 5]
        ...
        for i in range(0, bbox_target.size(0), B):
            ...
            target_xyxy = Variable(
                torch.FloatTensor(target.size())
            ...
            bbox_target_iou[i + max_index, torch.LongTensor([4]).cuda()] = (
                max_iou
            ).data.cuda()
        ...
        loss_xy = F.mse_loss(
            bbox_pred_response[:, :2], bbox_target_response[:, :2], reduction="sum"
        )
        loss_wh = F.mse_loss(
        ...
        loss_obj = F.mse_loss(
            bbox_pred_response[:, 4], target_iou[:, 4], reduction="sum"
        )
