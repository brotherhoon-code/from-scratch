    ...
    def __init__(
        ...
        **kwargs
        ...
        self.context_blocks.append(
            GlobalContextBlock(in_channels=64 * self.expansion)
        ) if 1 in n_context_stage_idx else self.context_blocks.append(nn.Sequential())

        self.stage2_block = self._make_stage(
            self.block_cls, 64 * self.expansion, 128, n_blocks[1], 2
        )
        self.context_blocks.append(
            GlobalContextBlock(in_channels=128 * self.expansion)
        ) if 2 in n_context_stage_idx else self.context_blocks.append(nn.Sequential())

        self.stage3_block = self._make_stage(
            self.block_cls, 128 * self.expansion, 256, n_blocks[2], 2
        )
        self.context_blocks.append(
            GlobalContextBlock(in_channels=256 * self.expansion)
        ) if 3 in n_context_stage_idx else self.context_blocks.append(nn.Sequential())

        self.stage4_block = self._make_stage(
            self.block_cls, 256 * self.expansion, 512, n_blocks[3], 2
        )
        self.context_blocks.append(
            GlobalContextBlock(in_channels=512 * self.expansion)
        ) if 4 in n_context_stage_idx else self.context_blocks.append(nn.Sequential())
