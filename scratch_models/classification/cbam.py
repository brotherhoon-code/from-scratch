...
class SpatialAttentionBlock(nn.Module):
    def __init__(self, kernel_size=7, activation_func="sigmoid"):
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.spatial_attn_block = SpatialAttentionBlock(
            kernel_size=kernel_size, activation_func=activation_func
        )
        self.channel_attn_block = ChannelAttentionBlock(
    ...
    def forward(self, x):
        ...
        channel_attn_map = repeat(
            self.channel_attn_block(x), "b c -> b c h w", h=x.size(2), w=x.size(3)
        )
        ...
        if spatial_attn_map.dim() == 1:  # e.g [2, 1, 1, 1]이나 squeeze에 의해 [2]로 표시됨
            spatial_attn_map = repeat(
                spatial_attn_map, "b -> b c h w", c=x.size(1), h=x.size(2), w=x.size(3)
            )
        else:
            spatial_attn_map = repeat(spatial_attn_map, "b h w -> b c h w", c=x.size(1))
