...
class GaussianNonLocalBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, r: int = 2):
        ...
        # g
        self.v_embed_layer = nn.Sequential(
            nn.Conv2d(
                in_channels=in_channels, out_channels=embed_dim, kernel_size=(1, 1)
            ),
            Rearrange("b c h w -> b (h w) c"),
        )  # B N c
        # W
        self.expansion_layer = nn.Conv2d(
            in_channels=embed_dim, out_channels=in_channels, kernel_size=(1, 1)
        )  # B C H W
...
class EmbeddedGaussianNonLocalBlock(nn.Module):
    def __init__(self, in_channels: int, r: int = 2):
        ...
        # theta
        self.q_embed_layer = nn.Sequential(
            nn.Conv2d(
                in_channels=in_channels, out_channels=embed_dim, kernel_size=(1, 1)
            ),
            Rearrange("b c h w -> b (h w) c "),
        ...
        # phi
        self.k_embed_layer = nn.Sequential(
            nn.Conv2d(
                in_channels=in_channels, out_channels=embed_dim, kernel_size=(1, 1)
            ),
    ...
    def forward(self, x):
        ...
        feature_map = rearrange(
            embedded_feature, "b (h w) c -> b c h w ", h=x.size(2), w=x.size(3)
        )  # B c H W
...
class ConcatNonLocalBlock(nn.Module):
    def __init__(self, in_channels: int, r: int = 2):
        ...
        # concat_proj
        self.concat_project_layer = nn.Sequential(
            nn.Conv2d(
    ...
    def forward(self, x):
        ...
        if (
            scaled_attn_score.dim() != 3
        ):  # 32x32 이미지에서 1x1 attn score의 경우 channel dim만 남게되므로 추가함
...
class NonLocalResNet(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.non_local_blocks.append(
            n_local_cls(in_channels=64 * self.expansion, r=2)
        ) if 1 in n_local_stage_idx else self.non_local_blocks.append(nn.Sequential())

        self.stage2_block = self._make_stage(
            self.block_cls, 64 * self.expansion, 128, n_blocks[1], 2
        )
        self.non_local_blocks.append(
            n_local_cls(in_channels=128 * self.expansion, r=2)
        ) if 2 in n_local_stage_idx else self.non_local_blocks.append(nn.Sequential())

        self.stage3_block = self._make_stage(
            self.block_cls, 128 * self.expansion, 256, n_blocks[2], 2
        )
        self.non_local_blocks.append(
            n_local_cls(in_channels=256 * self.expansion, r=2)
        ) if 3 in n_local_stage_idx else self.non_local_blocks.append(nn.Sequential())

        self.stage4_block = self._make_stage(
            self.block_cls, 256 * self.expansion, 512, n_blocks[3], 2
        )
        self.non_local_blocks.append(
            n_local_cls(in_channels=512 * self.expansion, r=2)
        ) if 4 in n_local_stage_idx else self.non_local_blocks.append(nn.Sequential())
