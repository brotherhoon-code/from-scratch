...
class PatchEmbeddingBlock(nn.Module):
    def __init__(
        ...
        self.flatten_layer = Rearrange(
            "b c (p_h h_n) (p_w w_n) -> b (h_n w_n) (c p_h p_w)",
            p_h=self.patch_size,
            h_n=self.n_patch_height,
            p_w=self.patch_size,
            w_n=self.n_patch_width,
        )
        self.linear_proj_layer = nn.Linear(
            self.patch_size * self.patch_size * self.img_channel, self.emb_dim
        )
...
class MultiHeadSelfAttentionBlock(nn.Module):
    def __init__(
        self, emb_dim: int, qkv_dim: int, num_heads: int = 8, dropout_ratio: float = 0.2
    ):
        ...
        self.rearrange_to_multi_head = Rearrange(
            "b n (h d) -> b h n d", h=self.num_heads
        )
        self.transpose_key = Rearrange("b h n d -> b h d n")
        self.rearrange_to_single_head = Rearrange(
            "b h n d -> b n (h d)", h=self.num_heads
        )
    ...
    def forward(self, x):
        ...
        scailed_attn_score = (
            torch.matmul(multi_query, transposed_multi_key) / self.root_key_dim
        )
        ...
        singlehead_features = self.rearrange_to_single_head(
            multihead_features
        )  # b h n d -> b n (h d)
...
class MlpBlock(nn.Module):
    def __init__(self, emb_dim: int, forward_dim: int, dropout_ratio: float = 0.2):
...
class EncoderBlock(nn.Module):
    def __init__(
        ...
        self.attention_layer = MultiHeadSelfAttentionBlock(
            emb_dim, qkv_dim, n_heads, dropout_ratio
        )
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        self.patch_embed_layer = PatchEmbeddingBlock(
            img_channel, img_height, img_width, patch_size, emb_dim
        )
        encoder_list = [
            EncoderBlock(emb_dim, qkv_dim, n_heads, hidden_dim, dropout_ratio)
